...
neighbors = list(product([0,1,-1], repeat=3))
...
while rowNum < len(dimMap):
    ...
    while columnNum < len(dimMap[0])-1:
        if dimMap[rowNum][columnNum] == '#':
            actives.add((rowNum, columnNum, 0))
        columnNum += 1
    rowNum += 1
...
while i < 6:
    ...
    for cube in actives:
        activeNeighbors = 0
        for n in neighbors:
            if (n[0]+cube[0], n[1]+cube[1], n[2]+cube[2]) in actives:
                activeNeighbors += 1
            else:
                inactives.add((n[0]+cube[0], n[1]+cube[1], n[2]+cube[2]))
    ...
    for iCube in inactives:
        activeNeighbors = 0
        for n in neighbors:
            if (n[0]+iCube[0], n[1]+iCube[1], n[2]+iCube[2]) in actives:
                activeNeighbors += 1
